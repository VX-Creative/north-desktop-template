...
# Limpa artefatos da cópia antes da primeira abertura do dev.
...
# Cria a solution final para o dev já abrir tudo pronto.
...
Write-Host "2. Restaurar os pacotes"
Write-Host "3. Compilar o projeto"

# Abre a pasta final pronta para o dev localizar a solution.
Start-Process explorer.exe $targetRootPath | Out-Null

# Limpa o pacote de entrega após a geração.
$cleanupCommand = "timeout /t 2 /nobreak >nul && del /f /q ""$packageZipPath"" && del /f /q ""$PSScriptRoot\New-Project.bat"" && del /f /q ""$PSCommandPath"""
